...
HALO Console v0.7.9 is a local-first Next.js and TypeScript application for chatting with Ollama models through a browser UI. The app keeps browser state, local API routes, model routing, document context, and manually selected learning context separated by explicit boundaries.
...
8. If Web Search is requested but not configured, the route returns a clear not-configured response.
9. The route streams from Ollama and forwards assistant text chunks to the browser.
...
- Scanned or image-only PDFs are reported as having no extractable text; OCR is not implemented in v0.7.9.
...
v0.7.9 includes Web Search foundation code but leaves it disabled and unconfigured by default.
